...
/* Problem description: Maior produto de quatro nUmeros adjacentes na mesma direção na grade
 *
 */
...
void pe11(void)
{
   ...
   ulong r=0; /* answer */
   int i, j; // contadores
   int Prod_H, Prod_V, DiagPri, DiagSec;
   int MaxProduct;
   ...
   for (i = 0; i < 20; i++)
   {
     for (j = 0; j < 17; j++)//pois são 4 nUmeros adjacentes
     {
     //inicia no ponto (i;j) da grade
       Prod_H = grade[i][j]*grade[i][j+1]*grade[i][j+2]*grade[i][j+3];
       if (Prod_H > MaxProduct) MaxProduct = Prod_H;
     }
   }

   // Produto vertical
   for (i = 0; i < 17; i++)//pois são 4 nUmeros adjacentes
   {
     for (j = 0; j < 20; j++)
     {
     //inicia no ponto (i;j) da grade
       Prod_V = grade[i][j]*grade[i+1][j]*grade[i+2][j]*grade[i+3][j];
       if (Prod_V > MaxProduct) MaxProduct = Prod_V;
     }
   }
   ...
   for (i = 0; i < 17; i++)//pois são 4 nUmeros adjacentes
   {
     ...
     {
     //inicia no ponto (i;j) da grade
       DiagPri = grade[i][j]*grade[i+1][j+1]*grade[i+2][j+2]*grade[i+3][j+3];
       if (DiagPri > MaxProduct) MaxProduct = DiagPri;
     }
   }
   ...
   for (i = 0; i < 17; i++)//pois são 4 nUmeros adjacentes
   {
     ...
     {
     //inicia no ponto (i;j) da grade
       DiagSec = grade[i][j]*grade[i+1][j-1]*grade[i+2][j-2]*grade[i+3][j-3];
