...
% World Setup:
%
% world_setup([Size, Type, Move, Gold, Pit, Bat, [RandS, RandA, GPS]]).
...
%   9. GPS: yes/no, allow use of GPS (not implemented)
%
% fig62:
%   gold: [2,3]
%   pits: [3,1], [3,3], [3,4]
...
% gps(GPS): GPS=[[Xpos,Ypos],Orient], give to agent the coordinates (column, line)
...
%get_setup([Size, Type, Move, Gold, Pit, Bat, [RandS, RandA, GPS]])
assert_setup :-
...
assert_setup :-
    ...
    get_setup(Lout),
    !,
    format('Reuse setup: Size=~w, Type=~w, Move=~w, Gold=~w, Pit=~w, Bat=~w, Adv=~w~n', Lout).

assert_setup :-
    Lout=[4, grid, stander, 0.1, 0.2, 0.1, [no, no]], % default
